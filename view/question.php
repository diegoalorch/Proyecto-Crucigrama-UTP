...
<div id="x2answer"></div>
<div class="modal fade bd-example-modal-lg" id="modalWin" tabindex="-1" role="dialog" aria-labelledby="modalWinLabel"
    aria-hidden="true">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="row" style="flex-wrap: nowrap !important;">
                    <img width="50%" src="<?php echo $GLOBALS['BASE_URL'] ?>publico/img/img/first_charcter_B.jpg" alt=""
                        srcset="">
                    <h3>Mejor suerte para la proxima</h3>
                </div>
            </div>
            <div class="modal-footer">
                <button type="button" onclick="borrarArray();location.reload()"
                    class="btn btn-primary">Continuar</button>
            </div>
        </div>
    </div>
</div>
<div class="modal fade bd-example-modal-lg" id="modalLost" tabindex="-1" role="dialog" aria-labelledby="modalLostLabel"
    aria-hidden="true">
    <div class="modal-dialog" role="document">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="row" style="flex-wrap: nowrap !important;">
                    <img width="50%" src="<?php echo $GLOBALS['BASE_URL'] ?>publico/img/img/first_character.png" alt=""
                        srcset="">
                    <h3>Increible respuesta correcta</h3>
                </div>
            </div>
            <div class="modal-footer">
                <button type="button" onclick="borrarArray();location.reload()" class="btn btn-secondary"
        ...
        while (letras_abecedario.length) {
            ...
            $(".pointter").on("click", function(event) {
                ...
                if (validarArray(event.target.textContent)) {
                    $(this).remove();
                    try {
                        document.querySelector("." + event.target.textContent).className =
                            "letra shadow p-1 bg-white rounded"
                    } catch (error) {
                        console.log(error)
                    }
                    validarRespuesta();
...
function condition_text(result) {
    ...
    if (result.answer.match(/,.*/)) {
        ...
        question.innerHTML = ("<span>" + result.question +
            "</span><br><br> Respuesta en orden alfabetico, ejemplo : La almendra, epidemia y los mamiferos");
        answer_with_comma();
        const button1 = document.createElement('button');
        document.getElementById('button_VA').appendChild(button1);
...
setTimeout(function() {
    ...
    var interval = setInterval(function() {
        ...
        $('.circle_animation').css('stroke-dashoffset', initialOffset - ((i + 1) * (initialOffset /
            time)));
        i--;
    }, 1000);
...
function nivel_complete() {
    Swal.fire({
        ...
        imageUrl: (nivel == "Facil") ?
            '<?php echo $GLOBALS['BASE_URL'] ?>publico/img/score/' + 1 + '-' + scoreFacil + '.png' : (nivel ==
                "Normal") ?
